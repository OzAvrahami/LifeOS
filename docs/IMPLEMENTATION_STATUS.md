...
- **#1 handoff status:** Open / Verify / P2 — Medium. Active candidate **0.4.0 (7)** implements Task reminder discovery from primary surfaces and relative Commitment reminders. Software, disposable PostgreSQL/Auth/RLS and Xcode Release compile gates pass; new production schema/API rollout and physical owner acceptance remain pending. See [current verification](issue-1-verification.md).
...
- **Current work item:** #1 Notifications MVP only. Build 5 failed native launch, never accepted; build 6 fixed startup and was superseded after owner feedback, not failed or accepted. Build 7 preserves Expo 57.0.23 / build-properties 57.0.20 / Notifications 57.0.12 and UIScene support. New migration `20260922120000` is locally verified, not remotely applied.
...
- **Last owner-accepted internal version:** **0.3.0/build 4**, visibly confirmed and owner-accepted on 2026-09-13; preparation baseline `e1a3470af233a1e10108f57eec73daed35551376`. No binary source SHA was extracted. Historical 0.2.1/build 3 acceptance and its completed preparation/publication checkpoints remain in [the 0.2.1 record](release-0.2.1-verification.md).
...
- **Next action:** Owner local Git checkpoint, then inspect/dry-run/authorize new schema rollout and verify history before API push/deployment; only then install and accept **0.4.0 (7)**. No 0.4.0 acceptance/publication is claimed.
...
| Notifications MVP (#1) | Software verified; rollout/physical acceptance pending | Task instants, Commitment relative leads, Weekly reminder and account preferences | 47 mobile suites (310 passed, 1 existing skip); 86 API tests; 13 local DB/RLS groups | Primary Task details access and Commitment defaults/editor/routing complete. Build 7 compiled, not installed. See [#1](issue-1-verification.md). No remote push or summaries. |
