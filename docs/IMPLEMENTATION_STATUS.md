...
- **#1 handoff status:** Open / Verify / P2 — Medium. Active candidate **0.4.0 (6)** corrects the Xcode 27 scene-lifecycle launch failure. Build 5 was built/installed but failed physical launch and was never accepted. Native scene/version/Pods synchronization, software checks and signed Xcode 27 Release compilation pass. Build 6 was installed/launched and its process survived at least 63 seconds; visual/notification owner acceptance remains pending. See [current verification](issue-1-verification.md).
...
- **Current work item:** #1 Notifications MVP compatibility correction only; no additional feature. Candidate **0.4.0 (6)** uses Expo 57.0.23 / build-properties 57.0.20 and UIScene support. Read-only remote history confirms migration 20260914120000; candidate API GitHub deployment status and live health pass, without direct provider inspection or a new production write test.
...
- **Next action:** Review the compatibility fix at the owner Git checkpoint and complete owner visual/notification acceptance on the installed **0.4.0 (6)**. Do not reuse failed build 5 or close #1 from compilation alone. No 0.4.0 acceptance/publication is claimed.
