...
זוג nullable של `time without time zone` המתאר טווח חוזר של שעות־קיר מקומיות ביום הפעיל של המשתמש. שתי העמודות הן `null` כאשר הטווח אינו מוגדר. הן חייבות להישמר או להימחק יחד, והתחלה וסיום שווים אינם תקינים.

אם `day_end_time` מוקדם מ־`day_start_time`, הסיום חל ביום הקלנדרי הבא. `00:00` הוא חצות תקינה. החלפת `timezone` אינה ממירה את שעות־הקיר השמורות.

הטווח אינו שעות עבודה, אינו זמן פנוי ואינו Capacity. הוא אינו משנה את שיוך `Task.planned_date`, את `DailyPlan.date` או את גבול היום הקלנדרי הקיים.

## default_daily_capacity_minutes (legacy compatibility)

שדה קיבולת ישן שנשמר לתאימות עם לקוחות מותקנים ועם DailyPlan קיים. הוא אינו מושג “היום שלי” ואינו משמש עוד כמכנה הראשי ב־Today.
...
אין להמיר אותו אוטומטית לשעות התחלה/סיום ואין למחוק ערכים קיימים.

## timezone

מזהה אזור זמן תקני מסוג IANA, לדוגמה:

```text
Asia/Jerusalem
```

בגרסה 0.1 אזור הזמן נשמר ב־UserSettings. אם עדיין אין רשומת הגדרות, היישום משתמש באזור הזמן של המכשיר כברירת מחדל אפקטיבית בלי ליצור רשומה אוטומטית.

כל חישובי Today ו־Week בזרימה הרגילה מתבססים על אזור הזמן האפקטיבי. התחייבויות נשמרות כתאריך ושעת־קיר מקומיים ואינן נכתבות מחדש כאשר אזור הזמן משתנה.

## סמנטיקת Settings API

`PUT /settings` ממשיך לדרוש את שלושת שדות התאימות (`defaultDailyCapacityMinutes`, `weekStartDay`, `timezone`). עבור חלון היום:

* השמטת `dayStartTime` וגם `dayEndTime` משמרת את הערכים הקיימים — זהו מסלול התאימות ללקוח ישן.
* שליחת שתי שעות `HH:mm` תקינות ושונות שומרת את הזוג באופן אטומי.
* שליחת שני הערכים כ־`null` מנקה את חלון היום.
* זוג חלקי, שילוב שעה ו־`null`, שעה לא תקינה או שעות שוות נדחים לפני כתיבה.

הבעלות ממשיכה להיגזר מה־JWT וה־RLS מגביל כל משתמש לשורת ההגדרות שלו. משתמש קיים ללא הערכים החדשים מקבל `null`/`null`; לא נוצרת עבורו תכנית שעות.
...
אם קיים ערך, הוא נכלל בסיכום זמן המשימות של Today.
...
* אם אין `DailyPlan`, או אם `available_minutes` הוא `null`, היום יורש את הערך הנוכחי של `UserSettings.default_daily_capacity_minutes`.
* כל ערך מספרי ב־`available_minutes`, כולל `0`, הוא חריגה מפורשת לאותו תאריך וגובר על ברירת המחדל.
* שינוי ברירת המחדל משפיע מיד על ימים שיורשים אותה, אך אינו מוחק חריגות יומיות מפורשות.
* שינוי מידע אחר של `DailyPlan`, כגון `focus_task_id`, חייב לשמר אם היום יורש את ברירת המחדל או משתמש בחריגה.

הסמנטיקה הזו נשמרת לתאימות ולעריכת DailyPlan: `null` הוא ירושה ו־`0` הוא חריגה מפורשת. Today אינו משתמש עוד בקיבולת האפקטיבית כמכנה ואינו מציג סטטוס עומס הנגזר ממנה.
...
# 23. סיכום זמן משימות ב־Today

בגרסה הנוכחית אין מספיק מידע כדי להסיק באופן אמין כמה זמן תכנון פנוי קיים ביום.
...
הסכום כולל Tasks משויכות לתאריך שאינן `cancelled`, לרבות Tasks פעילות והושלמו. ערך `estimated_minutes = null` נספר כהערכה חסרה ואינו מוסיף אפס מומצא לסכום. Commitments מוצגות בנפרד ואינן מחוברות לסכום Task Load. אין להפחית משכי Commitments או את אורך חלון היום כדי להפיק “זמן פנוי”.

שינוי חלון היום אינו משנה הערכות, סטטוסים, שיוך תאריכים, Daily Focus או סכום הזמן.
