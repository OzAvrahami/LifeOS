...
- **v0.4.0 / iOS build 5 candidate** — #1 first Notifications MVP: contextual permission, explicit Task reminders and a recurring local Weekly Planning reminder. Persisted account preferences/intent and device reconciliation are implemented; native preparation, schema/API rollout and physical acceptance are pending. No publication is claimed. Summary notifications, missed-task intelligence, smart suggestions and remote push remain future work. See [verification](issue-1-verification.md).
