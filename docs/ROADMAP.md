...
**Remaining work:** #4’s selected-week queries, task/commitment day inspection, day navigation, and editing/return context are implemented and await [physical-iPhone owner acceptance](issue-4-verification.md). This unreleased work does not change the completed v0.2.1 milestone. #3’s persisted lifecycle and four-step authenticated flow are implemented, with software checks and independent local PostgreSQL/Auth/RLS integration passing (all 11 groups). It is Open / Verify, as is #4. Their implementation checkpoint is pushed at `286cec4`. The shared **LifeOS 0.3.0 (4)** candidate is prepared with tracked and ignored native versions synchronized; build/install and [physical acceptance](issue-3-verification.md) remain pending after schema/API readiness is confirmed. No production migration or device acceptance is implied by local verification. Live #7/#8/#10 are Closed/Completed/Done and their accepted behavior is preserved. Other backlog work, including Google Calendar integration and #16, is unchanged. The 0.3.0 candidate is not a published release; Latest remains v0.2.1. The [canonical workflow](DEVELOPMENT_WORKFLOW.md) governs mandatory version/build review and separate publication.
