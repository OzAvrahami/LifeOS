...
- **v0.4.0 / iOS build 6 candidate** — #1 local Notifications MVP, with the supported Xcode 27 UIScene correction. Build 5 built/installed but failed native launch and was not accepted; build 6 now compiles, installs and survives native startup on the connected iOS 27 device; owner visual/notification acceptance remains pending. Native scene/version/Pods preparation and software checks are complete; remote migration history and historical API deployment status/live health have been checked. No publication is claimed. Summaries, missed-task intelligence, smart suggestions and remote push remain future work. See [verification](issue-1-verification.md).
