...
- **v0.4.0 / iOS build 7 candidate** — #1 Task reminder discovery and relative Commitment reminders join Weekly reminders in one local reconciler. Build 5 failed launch; build 6 fixed Xcode 27 startup and was superseded during product acceptance, not failed or accepted. Build 7 software/local DB checks and Release compile pass; new migration `20260922120000`, compatible API rollout and physical acceptance remain pending. No publication is claimed. Summaries, smart suggestions and remote push remain future work. See [verification](issue-1-verification.md).
